...
drone secret add --name maven_mirror_url --data 'http://nexus.infra:8081/repository/maven-public/' "${LINGUA_GREETER_GIT_REPO}"

drone secret add --name destination_image --data "${REGISTRY_NAME}/example/lingua-greeter" "${LINGUA_GREETER_GIT_REPO}"

drone secret add --name image_registry --data "${REGISTRY_NAME}" "${LINGUA_GREETER_GIT_REPO}"
